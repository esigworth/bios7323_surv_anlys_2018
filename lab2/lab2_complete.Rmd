...
```{r}
# Note: Weibull has different parameterization in K&M text and R
...
Right Censoring - event occurs after observed time $t$, $x \in (t,\infty)$

Left Censoring - event occurs before observed time $t$, $x \in (-\infty,t)$

Interval Censoring - event occurs after time $t_1$ and before time $t_2$, $x \in (t_1,t_2)$

Uncensored (Exact) - event occurs at observed time $t$, $x = t$
...
Note:  If the event time for an individual is censored, we have some information about when the event occurred. Under truncation, we don't have any information for people outside the "observable window". The event or censoring times for individuals who are observed are conditional on being in this window.
...
( mice10k<-as.tibble( list(t0=rexp(1e4,1)) ) %>% 
    mutate(censored=t0>2, t=if_else(censored,2,t0)) )

#mean and median for observed mice only (exclude censored times)
summ1 <- filter(mice10k,censored==FALSE) %>% summarise_at('t', c(mn1=mean,med1=median))

#mean and median ignoring censoring (treat censored time as event time)
summ2 <- summarise_at(mice10k, 't', c(mn2=mean,med2=median)) 

#get MLE of mean and median
summ3 <- mutate(mice10k,event=(censored==FALSE)) %>% 
  summarise_at(c('t','event'),sum) %>% 
  transmute(mnMLE=t/event,medMLE=log(2)*mnMLE)
...
chemo<-as.tibble( list(entry=runif(35,min=0,max=2), tte=rexp(35,rate=1)) ) %>%
  mutate(ctime=entry+tte, cens=ctime>2, ctime=if_else(cens,2,ctime), t=ctime-entry)

#plot showing staggered entry
ggplot(chemo, aes(x=ctime, y=1:35, shape=cens) )+
  xlab("calendar time") + ylab("participants")+
  geom_segment(aes(x=entry, y=1:35, xend=ctime, yend=1:35), linetype=3)+
  geom_point()

#plot w/ common time since entry
ggplot(chemo, aes(x=t, y=1:35, shape=cens) )+
  xlab("time since entry (years)") + ylab("participants")+
  geom_segment(aes(x=0, y=1:35, xend=t, yend=1:35), linetype=3)+
  geom_point()

#mean and median for observed (exclude censored times)
summ1 <- filter(chemo,cens==FALSE) %>% summarise_at('t', c(mn1=mean,med1=median))

#mean and median ignoring censoring (treat censored time as event time)
summ2 <- summarise_at(chemo, 't', c(mn2=mean,med2=median)) 

#get MLE of mean and median
summ3 <- mutate(chemo,event=(cens==FALSE)) %>% summarise_at(c('t','event'),sum) %>% transmute(mnMLE=t/event,medMLE=log(2)*mnMLE)
...
stroke<-as.tibble( list(t0=rexp(50,rate=1),c=rgamma(50,shape=4,scale=0.5))) %>%
  mutate(censored=c<t0, t=if_else(censored,c,t0))

#plot
ggplot(stroke, aes(x=t, y=1:50, shape=censored) )+
  xlab("time since entry") + ylab("participants")+
  geom_segment(aes(x=0, y=1:50, xend=t, yend=1:50), linetype=3)+
  geom_point()

#mean and median for observed (exclude censored times)
summ1<-filter(stroke,censored==FALSE) %>% summarise_at('t', c(mn1=mean,med1=median))

#mean and median ignoring censoring (treat censored time as event time)
summ2<-summarise_at(stroke, 't', c(mn2=mean,med2=median)) 

#get MLE of mean and median
summ3<-mutate(stroke,event=(censored==FALSE)) %>% summarise_at(c('t','event'),sum) %>% transmute(mnMLE=t/event,medMLE=log(2)*mnMLE)
...
```{r}
set.seed(4588)
dat <- as.tibble( list(t=rexp(250, 0.04)) )

#mean in original sample
mn1<-summarize_all(dat,mean)

#mean in only truncated data
trunc_age <- 20
mn2<-filter(dat, t>trunc_age) %>% summarize_all(mean)

#mean MLE = 1/lambda_hat 
# lambda_hat = n/sum(x_i - y_i)
# x_i is (truncated) event time and y_i is truncation time
mnMLE<-filter(dat, t>trunc_age) %>% mutate(num=1, denom=t-trunc_age) %>% 
  summarize_at(c('num','denom'),sum) %>% 
  transmute(lambda_hat=num/denom, mean=1/lambda_hat)

cbind(mn1,mn2,mnMLE['mean'])
```
